...
def learn_embeddings(walks, dimensions=128, window_size=10, workers=4, iter=1):
    '''
...
def learn_nodevec(nx_G, argp=1, argq=1, num_walks=10, walk_length=80, dimensions=128):
	'''
	Pipeline for representational learning for all nodes in a graph.
	'''
	G = Graph(nx_G, nx_G.is_directed(), argp, argq)
	G.preprocess_transition_probs()
	walks = G.simulate_walks(num_walks, walk_length)
	model = learn_embeddings(walks, dimensions=dimensions)
	return model


if __name__ == "__main__":
    nx_G = read_graph('data/karate.edgelist')
    model = learn_nodevec(nx_G, num_walks=100)
    model.save_word2vec_format('results/karate2.emd')
